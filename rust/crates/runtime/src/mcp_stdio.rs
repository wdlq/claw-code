...
mod tests {
    ...
    fn write_manager_mcp_server_script() -> PathBuf {
        ...
        .join("\n");
        fs::write(&script_path, script).expect("write script");
        // chmod 0o755 is Unix-only (`PermissionsExt::set_mode`). Windows
        // tests skip this — script executability on Windows is governed by
        // file extension + PATHEXT, not the mode bits.
        #[cfg(unix)]
        {
            let mut permissions = fs::metadata(&script_path).expect("metadata").permissions();
            permissions.set_mode(0o755);
    ...
    fn write_initialize_disconnect_script() -> PathBuf {
        ...
        .join("\n");
        fs::write(&script_path, script).expect("write script");
        // chmod 0o755 is Unix-only — Windows tests skip (see sample_bootstrap).
        #[cfg(unix)]
        {
            let mut permissions = fs::metadata(&script_path).expect("metadata").permissions();
            permissions.set_mode(0o755);
